...
interface FractionCompareResult {
  score: number;
  total: number;
  timeTaken: number;
  wrongQuestions: { question: string[]; userAnswer: number; correctAnswer: number }[];
}

export default function FractionCompareResultPage() {
  const router = useRouter();
  const [results, setResults] = useState<FractionCompareResult | null>(null);

  useEffect(() => {
    const storedResults = sessionStorage.getItem('fractionCompareResult');
    if (storedResults) {
      setResults(JSON.parse(storedResults));
    } else {
      // If no result is found, redirect back
      router.push('/fraction-compare');
    }
  }, [router]);
    ...
    <div className="min-h-screen bg-lightGreen">
      <div className="max-w-md mx-auto bg-white min-h-screen flex flex-col">
        ...
        <header className="p-6 pb-4">
          <h1 className="text-2xl font-bold text-darkText">分数比大小结果</h1>
        </header>
            ...
            <div className="bg-red-50 p-4 rounded-xl border border-red-200">
              ...
              {wrongQuestions.map((q, index) => (
                <div key={index} className="mb-4 last:mb-0 p-3 bg-white rounded-lg border border-red-100">
                  <p className="text-darkText mb-2">题目: {q.question.join(' vs ')}</p>
                  <div className="flex justify-between">
        ...
        <div className="p-6 border-t border-lightGreenAccent">
          <Link href="/fraction-compare" className="block w-full bg-primary text-white py-4 rounded-xl text-center font-medium hover:bg-opacity-90 transition">
            再试一次
          </Link>
